create extension postgis;

create table address_t (todofuken varchar(60),lat float,lon float,
    ttable varchar(40));

create table address_s (todofuken varchar(60),shikuchoson varchar(60),
    lat float,lon float );

create table address_o (todofuken varchar(60),shikuchoson varchar(60),
    ooaza varchar(60), tr_ooaza varchar(60),
    lat float, lon float );

create table address   (todofuken varchar(60),shikuchoson varchar(60),
    ooaza varchar(60), chiban varchar(60),
    lat float, lon float, geog geography('POINT') );
